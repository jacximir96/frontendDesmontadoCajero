...
import { DenominacionesBilletes, DenominacionBilleteConfirmado} from 'src/app/core/interfaces/shared';
import { BilletesService } from 'src/app/core/services/billetes.service';
...
export class TablaBilletesComponent implements OnInit{
  ...
  visibleTecladoInterno: boolean = false;
  ...
  direccion(flecha: string) {
    ...
    if(rowsInputs[currentIndexFocus].value != ''){
      ...
      rowsInputs[currentIndexFocus].placeholder = this.placeholder
    }
  ...
  redimensionar(ocultaTeclado = false){
    if (this.tablaBilletes) {
      ...
      if(ocultaTeclado){
        nativeElement.classList.remove('col-xl-7', 'col-lg-6', 'col-md-12', 'col-sm-12', 'col-12');
        nativeElement.classList.add('w-full','px-3');
      }else{
        nativeElement.classList.remove('w-full','px-3');
        nativeElement.classList.add('col-xl-7', 'col-lg-6', 'col-md-12', 'col-sm-12', 'col-12');
      }
